      ...
      <aside className="hidden w-60 flex-col border-r border-slate-200 bg-white lg:flex">
        ...
        <div className="px-5 pt-4">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-cyan-200 bg-cyan-50 px-2.5 py-1 text-xs font-semibold text-cyan-700">
            <Compass className="h-3.5 w-3.5" /> IT mode
          </span>
          {user?.full_name && (
              ...
              <div className="space-y-1">
                {group.items.map((n) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        "flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300",
                        isActive
        ...
        <div className="border-t border-slate-100 p-4 text-xs text-slate-500">
          <div className="rounded-xl bg-cyan-50/60 p-3 text-cyan-800">
            IT setup tasks belong to the new hire's onboarding instance —
            HR and the manager see the same status you set here.
          </div>
        </div>
      </aside>
        ...
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-slate-200 bg-white/85 px-6 backdrop-blur">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              className="rounded-xl border border-slate-200 p-2 text-slate-500 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
              aria-label="Log out"
            >
